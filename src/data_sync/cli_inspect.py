    ...
    with cdflib.CDF(str(file_path)) as cdf:
        ...
        for var_name in all_vars:
            try:
                ...
                console.print(f"[red]Warning: Could not read variable {var_name}: {e}[/red]")
        ...
        for var_name, data, num_recs in var_info_list:
            ...
            if isinstance(data, np.ndarray):
                ...
                if len(data.shape) == 1:
                    ...
                    for col_idx in range(min(data.shape[1], 10)):  # Limit to 10 columns
                    ...
                    for i in range(min(num_records, num_recs)):
                        row_values = [str(i)]
                        for col_idx in range(min(data.shape[1], 10)):
                            row_values.append(f"{data[i, col_idx]:.4g}")
                        if data.shape[1] > 10:
                            row_values.append("...")
